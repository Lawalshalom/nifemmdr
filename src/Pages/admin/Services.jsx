	...
	const getAllPosts = () => {
		...
		Axios.get(`${baseUrl}/api/posts/all`, { headers })
			.then((res) => {
				setLoading(false);
				if (res.data.posts) {
					const servicesPosts = res.data.posts.filter(
						(post) => post.page === "services"
					);
					setPosts(servicesPosts);
			...
			{posts.map((post) => (
				<div key={post.id}>
					<h2>
						...
						{post.header === "New Services Page Post" && (
							<DeleteOutlined
								style={{ float: "right" }}
				...
				<Modal
					...
					<Form form={form} layout="vertical">
						<Form.Item
							name="header"
							label="Post title"
							initialValue="New Services Page Post"
							rules={[

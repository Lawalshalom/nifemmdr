	...
	const getAllPosts = () => {
		...
		Axios.get(`${baseUrl}/api/posts/all`, { headers })
			.then((res) => {
				setLoading(false);
				if (res.data.posts) {
					const contactPosts = res.data.posts.filter(
						(post) => post.page === "contact"
					);
					setPosts(contactPosts);
			...
			{posts.map((post) => (
				<div key={post.id}>
					<h2>
						...
						{post.header === "New Contact Page Post" && <DeleteOutlined
							style={{ float: "right" }}
				...
				<Modal
					...
					<Form form={form} layout="vertical">
						<Form.Item
							name="header"
							label="Post title"
							initialValue="New Contact Page Post"
							rules={[
